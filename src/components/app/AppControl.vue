<template>
    <div>
        <div>
            <div>
                <!-- <input type="text" :value="appConfig.mainConfig.width">
                <input type="text" :value="appConfig.mainConfig.height"> -->
                <Input type="number" :label="'Width'" input-id="width" :get-value="getValue" :set-value="setValue"/>
                <Input type="number" :label="'Height'" input-id="height" :get-value="getValue" :set-value="setValue"/>
                <!-- <Input type="number" :label="'height'" :value="appConfig.mainConfig.height" :value-change="valueChange"/> -->

                color picker
                <div>
                    Shape
                    <div>
                        Regular Shapes
                        <button style="display: block;" @click="focusObject(appConfig.listLayer.currentLayer.addObject('Rect'))">Rectangle</button>
                        <button style="display: block;" @click="focusObject(appConfig.listLayer.currentLayer.addObject('Ellipse'))">Ellipse</button>
                        <button style="display: block;" @click="focusObject(appConfig.listLayer.currentLayer.addObject('Circle'))">Circle</button>
                        <button style="display: block;" @click="focusObject(appConfig.listLayer.currentLayer.addObject('Wedge'))">Wedge</button>
                        <button style="display: block;" @click="focusObject(appConfig.listLayer.currentLayer.addObject('Star'))">Star</button>
                        <button style="display: block;" @click="focusObject(appConfig.listLayer.currentLayer.addObject('RegularPolygon'))">RegularPolygon</button>
                    </div>
                    <div>
                        Line Polygon
                        <button style="display: block;" @click="drawLine()">Draw Line</button>
                        {{appConfig == null ? "" : appConfig.mode}}
                        <button style="display: block;" @click="drawPolygon()" v-if="appConfig && appConfig.mode != 'draw-polygon'">Draw Polygon</button>
                        <button style="display: block;" @click="stopDrawPolygon()" v-if="appConfig && appConfig.mode == 'draw-polygon'">Stop Draw Polygon</button>
                    </div>
                    <div>
                        Image
                        <button style="display: block;" @click="loadImage()">Load Image</button>
                        <button style="display: block;" @click="loadImageFromCollection()">Load Image From Collection</button>
                        <input type="text" v-model="urlImage">
                        <button style="display: block;" @click="loadImage(urlImage)">Load Image</button>
                    </div>
                    <div>
                        Text
                        <input type="text" v-model="newText">
                        <button style="display: block;" @click="focusObject(appConfig.listLayer.currentLayer.addObject('Text', {text: newText || 'Default Text'}));newText=''">Add Text</button>
                    </div>
                </div>
                <div>
                    layer
                    <input type="text" v-model="newLayerName">
                    <button @click="appConfig.listLayer.addLayer(newLayerName);newLayerName=''">Add layer</button>
                    <div v-for="(layer, key) in _getListOfLayers" :key="layer.attrs.name + key">
                        {{layer.attrs.name}} {{key}} <button>set current layer</button>
                    </div>
                    <div>
                        Background Layer
                        <button>Set current layer</button>
                    </div>
                </div>
                <div>
                    multiple Select
                    <button @click="multipleSelect">Select</button>
                </div>
            </div>
        </div>
        <!-- <div>
            AppProperties
            <AppProperties v-if="appConfig != null && appConfig.currentFocus != null" :shape="appConfig.currentFocus"></AppProperties>
        </div> -->
    </div>
</template>

<script>
import { Shape } from 'konva'
import { setCurrentFocusedObject } from '../../services/utils'
import AppProperties from './AppProperties'
import Input from '../misc-component/input'

export default {
    name: 'Control',
    components: {
        AppProperties,
        Input
    },
    props: {
        appConfig: {
            type: Object,
            default() {
                return {
                        mainConfig: {
                            width: 0,
                            height: 0
                        },
                        listLayer: []
                    }
            }
        }
    },
    data() {
        return {
            urlImage: null,
            newLayerName: null,
            newText: ""
        }
    },
    methods: {
        getLayerDataSource() {
            return this.appConfig.listLayer.layerList
        },
        removeLayer(layer) {
            this.appConfig.listLayer.removeLayer
            this.appConfig.currentFocus = null
        },
        isShape(object) {
            return object instanceof Shape
        },
        onCurrentLayerChange(newLayer = null) {
            setCurrentFocusedObject(this.appConfig, newLayer)
        },
        focusObject(object) {
            setCurrentFocusedObject(this.appConfig, object)
        },
        drawLine() {
            const _newLine = this.appConfig.listLayer.currentLayer.addObject( "Line", {
                    strokeEnabled: true,
                    stroke: "black",
                    fillEnabled: false,
                    strokeWidth: 5,
                    width: 1,
                    height: 1
                }
            );
            this.appConfig.mode = "draw-line";
            setCurrentFocusedObject(this.appConfig, _newLine);
        },
        drawPolygon() {
            const _newLine = this.appConfig.listLayer.currentLayer.addObject("Line", {
                    strokeEnabled: true,
                    stroke: "black",
                    fillEnabled: true,
                    strokeWidth: 10,
                    closed: true,
                    width: 1,
                    height: 1,
                    draggable: false
                }
            );
            this.appConfig.mode = "draw-polygon";
            setCurrentFocusedObject(this.appConfig, _newLine);
        },
        stopDrawPolygon() {
            this.appConfig.mode = null;
            this.appConfig.stage.draggable(true);
            this.appConfig.stage.container().style.cursor = "auto";
            const _currObj = this.appConfig.currentFocus;
            if(_currObj && _currObj.getClassName() == "Line") {
                _currObj.points(_currObj.tempPoints);
                _currObj.draggable(true);
                this.focusObject(_currObj);
            }
        },
        loadImage(imageURL, imageName) {
            let self = this;
            if(imageURL) {
                console.log(imageURL)
                this.appConfig.listLayer.currentLayer.addImage(imageURL, {name: imageName});
            } else {
            const inputEle = document.createElement("input");
            inputEle.type = "file";
            inputEle.accept = "image/*";

            inputEle.addEventListener('change', () => {
                const file = inputEle.files[0];
                if (!file) return;
                if (!self.appConfig.listLayer.currentLayer) return;
                
                const payload = new FormData();
                payload.append("file", file, file.name);
                
                // this.simApiService.createImage(payload)
                // .subscribe(res => {
                //     this.appConfig.listLayer.currentLayer.addImage(res.content.path, { name: res.content.name});
                //     this.spinner.hide();
                // })
                self.appConfig.listLayer.currentLayer.addImage("data:image/jpeg;base64,", { name: "boy"});
            })
            inputEle.click();
            }
        },
        layerMoveDown(event) {
            const layer = event.item.data;
            if(layer) {
                const _length = this.appConfig.listLayer.layerList.length;
                const _newZIndex = _length - event.currentIndex;
                layer.setZIndex(_newZIndex);
                this.appConfig.stage.batchDraw();
            }
        },
        // loadImageFromCollection() {
        //     const dialogRef = this.dialog.open(SimLoadImageDialogComponent);
        //     dialogRef.afterClosed().subscribe((imgs) => {
        //     if(imgs && imgs.length) {
        //         imgs.forEach(img => (this.loadImage(img.path, img.name)));
        //     }
        //     })
        // },
        getListOfLayers() {
            // if(!this.appConfig) return []
            return this.appConfig.listLayer.layerList.sort((la, lb) => -la.getZIndex() + lb.getZIndex());
        },
        multipleSelect() {
            this.appConfig.mode = "multiple-select"
        },
        getValue(param, inputId) {
            return this.appConfig.mainConfig[param || inputId]
        },
        setValue(param, value, inputId) {
            // if(value < 1080) return
            this.appConfig.mainConfig[param || inputId] = value
            this.appConfig.updateMainConfig()
        }
    },
    computed: {
        _getListOfLayers() {
            // if(!this.appConfig) return []
            return this.appConfig.listLayer.layerList
        }
    }
}
</script>